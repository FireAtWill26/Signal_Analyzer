...
@TRAINING_REGISTRY.register('Feature_Analysis')
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        self.cfg_list = [deepcopy(cfg) for _ in range(self.training_dic["num_buckets"])]
        ...
        if self.training_dic["eval_mode"] == "recent":
            ...
            pos_holders = [np.zeros(indices[1].shape).astype(bool) for _ in range(self.training_dic["num_buckets"])]
            ...
            eval_indices = [np.zeros(indices[1].shape).astype(bool) for _ in range(self.training_dic["num_buckets"])]
            pos_holders = [np.zeros(indices[1].shape).astype(bool) for _ in range(self.training_dic["num_buckets"])]
        ...
        self.train_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], train_indices[i], self.train_output,date_weight,feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}
        self.eval_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], eval_indices[i], self.train_output,date_weight,feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}
        ...
        self.eval_dataset = SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], eval_indices[0], self.train_output,date_weight,feature_matrix=self.feature_matrix)

        self.train_loader_list = {i:DataLoader(self.train_dataset_list[i], batch_size=self.training_dic["batch_size"], shuffle=True, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.training_dic["num_buckets"])}
        self.eval_loader_list = {i:DataLoader(self.eval_dataset_list[i], batch_size=self.training_dic["batch_size"]*8, shuffle=False, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.training_dic["num_buckets"])}
        ...
        if ("finetune" in self.training_dic and self.training_dic["finetune"]) or ("discrimination" in self.training_dic and self.training_dic["discrimination"]):
            ...
            finetune_train_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], finetune_train_indices, self.train_output,np.ones(days),feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}
            finetune_eval_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], finetune_eval_indices, self.train_output,np.ones(days),feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}

            self.finetune_train_loader_list = {i:DataLoader(finetune_train_dataset_list[i], batch_size=self.training_dic["batch_size"], shuffle=True, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.training_dic["num_buckets"])}
            self.finetune_eval_loader_list = {i:DataLoader(finetune_eval_dataset_list[i], batch_size=self.training_dic["batch_size"], shuffle=False, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.training_dic["num_buckets"])}
        ...
        for i in range(self.training_dic["num_buckets"]):
            self.cfg_list[i].training.model_params["eval_steps"] = len(self.train_loader_list[i]) // self.training_dic["step_num"]
        
        # self.training_dic = self.cfg_list[0].training.model_params
        self.training_dic["lr"] *= self.training_dic["num_buckets"]
        self.training_dic["retrain_lr"] *= self.training_dic["num_buckets"]
        ...
        self.model_list = {i:create_model(self.cfg_list[i].model) for i in range(self.training_dic["num_buckets"])}
        ...
        if "regression_method" in self.training_dic and self.training_dic["regression_method"] == "Lasso":
            self.linear_regressor = Lasso(alpha=self.training_dic["regressor_alpha"], positive=True, fit_intercept=False)
        elif "regression_method" in self.training_dic and self.training_dic["regression_method"] == "Ridge":
            self.linear_regressor = Ridge(alpha=self.training_dic["regressor_alpha"], positive=True, fit_intercept=False)
        else:
        ...
        if "regression_only" not in self.training_dic or not self.training_dic["regression_only"]:
            ...
            pbar = tqdm(range(self.training_dic['num_buckets']), desc='Training', ncols=200)
            ...
            if self.training_dic['opt'] == "Adam":
                ...
                } for j in range(len(parameter_groups[i]))]) for i in range(self.training_dic["num_buckets"])]
                ...
                for i in range(self.training_dic["num_buckets"]):
                    muon_params_list = [[p for p in self.model_list[i].parameters() if p.ndim >= 2] for i in range(self.training_dic["num_buckets"])]
                    adamw_params_list = [[p for p in self.model_list[i].parameters() if p.ndim < 2] for i in range(self.training_dic["num_buckets"])]
                self.optimizer_list = [MuonWithAuxAdam(dict(params=muon_params_list[i], use_muon=True, lr=lr, weight_decay=self.weight_decay), dict(params=adamw_params_list[i], use_muon=False, lr=lr/10, weight_decay=self.weight_decay)) for i in range(self.training_dic["num_buckets"])]
            ...
            for model_idx in pbar:
                ...
                for epoch in range(self.training_dic["epochs"]):
                    ...
                    for data in self.train_loader_list[model_idx]:
                        ...
                        if step_cnt == self.cfg_list[model_idx].training.model_params["eval_steps"]:
                            ...
                            pbar.set_description("Model :%d|Epoch :%d|Train Loss: %.2e|Evaluation Loss: %.2e|MSE Loss: %.2e|Correlation: %.2e|LR: %.2e|Retrain_LR: %.2e" % (model_idx+1, epoch+1, train_loss, eval_loss, mse_loss, correlation_loss, self.optimizer.param_groups[0]["lr"], self.training_dic["retrain_lr"]))
                            ...
                            if (self.training_dic["retrain"] and overfit_cnt >= self.training_dic["overfit_patience"]) or (self.optimizer.param_groups[0]["lr"] <= self.training_dic["retrain_lr"]/5e2):
                                ...
                                for i in range(len(self.optimizer_list[model_idx].param_groups)):
                                    self.optimizer_list[model_idx].param_groups[i]["lr"] = self.training_dic["retrain_lr"]
                                    if i > 0:
                ...
                del self.train_loader_list[model_idx], self.train_dataset_list[model_idx]
                gc.collect()
                torch.cuda.empty_cache()
                print("\n")
            ...
            if "discrimination" in self.training_dic and self.training_dic["discrimination"]:
                for model_idx in range(self.training_dic["num_buckets"]):                
                    ...
                    del self.finetune_train_loader_list[model_idx], finetune_train_dataset_list[model_idx], self.finetune_eval_loader_list[model_idx], finetune_eval_dataset_list[model_idx]
                    gc.collect()
                    torch.cuda.empty_cache()
        ...
        if "regressor_type" in self.training_dic and self.training_dic["regressor_type"] == "nn.linear":        
            ...
            if "equal_weights" in self.training_dic and self.training_dic["equal_weights"]:
                del self.regressor_eval_loader, self.regressor_train_loader, regressor_eval_dataset, regressor_train_dataset
                self.linear_coef = np.ones(self.training_dic["num_buckets"]) / self.training_dic["num_buckets"]
                ...
                if "weight_essential" in self.training_dic and self.training_dic["weight_essential"]:
                    ...
                    sample_weights += F.sigmoid(8 * (pred - pred_mean - self.training_dic["mask_pos"] * pred_std)) * self.training_dic["weight_pow_under"]
    ...
    def feature_bucketing(self, cfg):
        ...
        # if Path(self.feature_buckets_loc).exists():
        #     feature_buckets_indices = np.load(self.feature_buckets_loc)
        #     saved_features = feature_buckets_indices[0].shape[0]
        #     # import ipdb; ipdb.set_trace()
        #     if saved_features == features_num and len(feature_buckets_indices) == self.training_dic["num_buckets"]:
        ...
        for key in turn_over:
            alphas_turnover[alpha_pos_name[alpha_indices[key]]] = turn_over[key]

        sorted_alphas = sorted([(key, alphas_turnover[key]) for key in alphas_turnover], key=lambda x: x[1])
