...
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        self.train_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], train_indices, self.train_output,date_weight,feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}
        self.eval_dataset_list = {i:SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.feature_buckets[i], eval_indices, self.train_output,date_weight,feature_matrix=self.feature_matrix) for i in range(self.training_dic["num_buckets"])}
        ...
        self.eval_loader_list = {i:DataLoader(self.eval_dataset_list[i], batch_size=self.training_dic["batch_size"], shuffle=False, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.training_dic["num_buckets"])}
        ...
        self.training_dic["eval_steps"] = len(self.train_loader_list[0]) // self.training_dic["step_num"]
        ...
        if cfg.model.type == "KANsformer":
            ...
            self.cfg.model.model_params["expand_dim"] = embed_dim * 2
                ...
                for epoch in range(self.training_dic["epochs"]):
                    ...
                    for data in self.train_loader_list[model_idx]:
                        ...
                        if step_cnt == self.training_dic["eval_steps"]:
                            ...
                            if (self.training_dic["retrain"] and overfit_cnt >= self.training_dic["overfit_patience"]) or (epoch + 1 == self.training_dic["epoch"] // 3 and retrain_cnt == 0):
                                skip_reset = False
                                overfit_cnt = 0
