...
class KANGRU(torch.nn.Module):
    def __init__(
        ...
        embedding_layers = [input_dim, math.ceil(input_dim * 1.5), hidden_dim]

        self.Embeddings = nn.ModuleList([BuildLayer(in_dim, out_dim, grid_min, grid_max, num_grids_in, use_base_update, base_activation, spline_weight_init_scale, denominator, self.dropout_rate, use_layernorm, radial_type, mode=mode, device=device, infer=infer) for in_dim, out_dim in zip(embedding_layers[:-1], embedding_layers[1:])])
        ...
        for idx in range(num_layers):
            ...
            LayerDict["kan_z"] = BuildLayer(concat_dim, hidden_dim, grid_min, grid_max, num_grids, use_base_update, base_activation, spline_weight_init_scale, denominator, self.dropout_rate, use_layernorm, radial_type, mode=mode, device=device, infer=infer)
            LayerDict["kan_r"] = BuildLayer(concat_dim, hidden_dim, grid_min, grid_max, num_grids, use_base_update, base_activation, spline_weight_init_scale, denominator, self.dropout_rate, use_layernorm, radial_type, mode=mode, device=device, infer=infer)
            LayerDict["cand_h"] = BuildLayer(concat_dim, hidden_dim, grid_min, grid_max, num_grids, use_base_update, base_activation, spline_weight_init_scale, denominator, self.dropout_rate, use_layernorm, radial_type, mode=mode, device=device, infer=infer)
        ...
        out_layers = [hidden_dim, 32, 1]

        self.out = nn.ModuleList([BuildLayer(in_dim, out_dim, grid_min, grid_max, num_grids_out, use_base_update, base_activation, spline_weight_init_scale, denominator, self.dropout_rate, use_layernorm, radial_type, mode=mode, device=device, infer=infer) for in_dim, out_dim in zip(out_layers[:-1], out_layers[1:])])
    ...
    def forward(self, input, h0=None, infer=None):
        ...
        for idx in range(self.num_layers):
            ...
            output = current_input.squeeze(-1)
