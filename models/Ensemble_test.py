...
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        if self.training_dic.get("validation_by_return", False) or "ret" in self.training_dic.get("loss_fn"):
            if self.training_dic["eval_mode"] not in ["by_date", "recent", "period"]:
                self.training_dic["eval_mode"] = "recent"
        ...
        if "ret" in self.training_dic.get("loss_fn"):
            self.train_dataset_list = [SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], train_indice & self.stocks_choice[i], self.train_output, 1, feature_matrix={"features": feature_matrix, "ret_ratio": self.ret_ratio}, time_as_batch=True) for i in range(self.num_labels)]
            ...
            self.train_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], train_indice & self.stocks_choice[i], self.train_output, date_weight,feature_matrix=feature_matrix) for i in range(self.num_labels)]
        ...
        if self.training_dic.get("validation_by_return", False):
            self.eval_dataset_list = [SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], eval_indice & self.stocks_choice[i], self.train_output, 1, feature_matrix={"features": feature_matrix, "ret_ratio": self.ret_ratio}, time_as_batch=True) for i in range(self.num_labels)]
            self.eval_loader_list = [DataLoader(self.eval_dataset_list[i], batch_size=1, shuffle=False, drop_last=False, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.num_labels)]
        else:
            self.eval_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], eval_indice & self.stocks_choice[i], self.train_output, date_weight,feature_matrix=feature_matrix) for i in range(self.num_labels)]
        ...
        if ("finetune" in self.training_dic and self.training_dic["finetune"]) or ("discrimination" in self.training_dic and self.training_dic["discrimination"]):
            ...
            finetune_train_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], finetune_train_indices & self.stocks_choice[i], self.train_output,np.ones(days),feature_matrix=feature_matrix) for i in range(self.num_labels)]
            finetune_eval_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], finetune_eval_indices & self.stocks_choice[i], self.train_output,np.ones(days),feature_matrix=feature_matrix) for i in range(self.num_labels)]
        ...
        regressor_train_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], regressor_train_indices & self.stocks_choice[i], self.train_output,np.ones(days),feature_matrix=feature_matrix) for i in range(self.num_labels)]
        regressor_eval_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], regressor_eval_indices & self.stocks_choice[i], self.train_output,np.ones(days),feature_matrix=feature_matrix) for i in range(self.num_labels)]
        regressor_total_dataset_list = [SharedMemDataset(model_name, self.train_input.shape, self.train_input.dtype, self.indices[0], regressor_total_indices & self.stocks_choice[i], self.train_output,np.ones(days),feature_matrix=feature_matrix) for i in range(self.num_labels)]
        ...
        if "regression_only" not in self.training_dic or not self.training_dic["regression_only"]:
            ...
            for i in range(self.num_buckets):
                for j in range(self.num_labels):
                    ratio = (indices[0].sum()/self.feature_buckets_list[j][i].sum()) ** (0.5)
                ...
                for model_idx in range(self.num_buckets):
                    ...
                    for epoch in pbar:
                        ...
                        for data in self.train_loader_list[label_idx]:
                            ...
                            # 提前批量移动数据到设备
                            if "ret" in self.training_dic.get("loss_fn"):
                                extra_batch = data[-1][0].squeeze().transpose(1,0)
                            else:
                                extra_batch = data[-1]
                            if self.training_dic.get("reduce_low_cap", False):
                            ...
                            if "ret" in self.training_dic.get("loss_fn"):
                                ret_ratio = data[-1][1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype)
                            if self.training_dic["denormalizer"]:
                            ...
                            if self.training_dic["opt"] == "LBFGS":
                                ...
                                if "ret" in self.training_dic.get("loss_fn"):
                                    ...
                                    for i in range(math.ceil(data_batch.shape[0] / self.training_dic["batch_size"])):
                                        ...
                                        y_chunk = torch.squeeze(self.model.forward(data_batch[self.training_dic["batch_size"]*i: self.training_dic["batch_size"]*(i+1),self.feature_select_list[label_idx][model_idx]]))
                                ...
                                if "discrimination" in self.training_dic and self.training_dic["discrimination"]:
                                    if "ret" not in self.training_dic.get("loss_fn"):
                                        ...
                                        while unfinished_len > 0:
                                            ...
                                            dis_pred = torch.cat([dis_pred, pred[pos:len_diff+pos]], dim=0)
                                            dis_data = torch.cat([dis_data, data_batch[pos:len_diff+pos,self.feature_select_list[label_idx][model_idx]]], dim=0)
                                            dis_target = torch.cat([dis_target, target_batch[pos:len_diff+pos]], dim=0)
                                            pos=len_diff+pos
                                ...
                                if self.training_dic.get("validation_by_return", False):
                                    with torch.no_grad():
                                        ...
                                        for data in self.eval_loader_list[label_idx]:
                                            # import ipdb; ipdb.set_trace()
                                            if self.training_dic.get("reduce_low_cap", False):
                                                extra_feat = data[-1][0].squeeze().transpose(1,0)[:, :-1]
                                                reduction_cap = data[-1][0].squeeze().transpose(1,0)[:, -1]
                                            else:
                                                extra_feat = data[-1][0].squeeze().transpose(1,0)
                                            if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                    ...
                                    if not result[(label_idx,model_idx)]["return"] or total_ret > highest_ret:
                                        # best_correlation_loss = correlation_loss
                                        # if eval_loss < best_eval_loss:
                                        highest_ret = total_ret
                    ...
                    info = f"Label {label_idx} Model {model_idx} is saved on epoch {current_time_stamp[0]+1} step {current_time_stamp[1]+1}"
                    print(info)
    ...
    def evaluation(self, cfg_list):
        ...
        with torch.no_grad():
            for label_idx in range(self.num_labels):
                ...
                for data in self.eval_loader_list[label_idx]:
                    if self.training_dic.get("validation_by_return", False):
                        ret_ratio = data[-1][1].squeeze()
                        extra_batch = data[-1][0]
                    else:
                        extra_batch = data[-1]
                    if self.training_dic.get("reduce_low_cap", False):
                        extra_feat = extra_batch[:, :-1].squeeze()
                        reduction_cap = extra_batch[:, -1].squeeze()
                    else:
                        extra_feat = extra_batch.squeeze()
                    if self.training_dic.get("validation_by_return", False):
                        extra_feat = extra_feat.transpose(1,0)
                    if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                        data_batch = torch.cat([data[0].squeeze(), extra_feat], dim=1).to(self.device, non_blocking=True, dtype=data[0].dtype)
                    else:                    
                        data_batch = data[0].squeeze().to(self.device, non_blocking=True)
    ...
    def predict(self, x, cfg, date=None, test_stage=True, lock=None, valid_stock=None, *args, **kwargs):
        ...
        if self.training_dic.get("validity_for_infer", False) and Path(self.stat_loc).exist():
