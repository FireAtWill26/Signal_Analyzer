...
@TRAINING_REGISTRY.register('Retrain')
        ...
        for epoch in pbar:
            ...
            with torch.no_grad():
                for data_batch, target_batch in eval_loader:
                    ...
                    eval_loss += loss_fn_eval(torch.squeeze(pred), target_batch).item()
                    mse_loss += nn.MSELoss()(torch.squeeze(pred), target_batch).item()
            ...
            pbar.set_description("Epoch :%d | Train Loss: %.2e | Evaluation Loss: %.2e | MSE Loss: %.2e" % (epoch+1, train_loss, eval_loss, mse_loss))
