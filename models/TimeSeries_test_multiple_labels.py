...
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        extra_info = {"features": feature_matrix}
        ...
        for i in range(self.num_labels):            
            if self.training_dic.get("cross_section", False):
                if use_endpoints_eval:
                    self.train_dataset_list.append(SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, indices[0], train_indices & self.stocks_choice[i], self.train_output, self.seq_len, feature_matrix=extra_info, time_as_batch=True))
                else:
                    self.train_dataset_list.append(SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, indices[0], train_indices & self.stocks_choice[i], self.train_output, self.seq_len, feature_matrix=extra_info, time_as_batch=True))
            else:
                if use_endpoints:
            ...
            if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
                if use_endpoints_eval:
                    self.eval_dataset_list.append(SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, indices[0], eval_indices & self.stocks_choice[i], self.train_output, self.seq_len, feature_matrix=extra_info, time_as_batch=True))
                else:
                    self.eval_dataset_list.append(SharedMemSeqDatasetDaily(model_name, self.train_input.shape, self.train_input.dtype, indices[0], eval_indices & self.stocks_choice[i], self.train_output, self.seq_len, feature_matrix=extra_info, time_as_batch=True))
            else:
                if use_endpoints_eval:
        ...
        if self.training_dic.get("cross_section", False):
            self.train_loader_list = [DataLoader(self.train_dataset_list[i], batch_size=1, shuffle=True, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.num_labels)]
        else:
        ...
        if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
            self.eval_loader_list = [DataLoader(self.eval_dataset_list[i], batch_size=1, shuffle=False, drop_last=True, prefetch_factor=4, pin_memory=True, num_workers=num_workers, persistent_workers=True) for i in range(self.num_labels)]
        else:
        ...
        if ("discrimination" in self.training_dic and self.training_dic["discrimination"]) or ("finetune" in self.training_dic and self.training_dic["finetune"]):
            ...
            finetune_train_dataset_list = [SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], train_indices & self.stocks_choice[i], self.train_output, self.seq_len, np.zeros(days), True, train_endpoints & self.stocks_choice[i], feature_matrix) for i in range(self.num_labels)]
            finetune_eval_dataset_list = [SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], eval_indices & self.stocks_choice[i], self.train_output, self.seq_len, np.zeros(days), True, eval_endpoints & self.stocks_choice[i], feature_matrix) for i in range(self.num_labels)]
        ...
        for label_idx in range(self.num_labels):
            ...
            if self.training_dic.get("cross_section", False):
                self.eval_steps = len(self.train_dataset_list[label_idx]) // self.training_dic["step_num"]
            else:
                self.eval_steps = len(self.train_dataset_list[label_idx]) // (self.training_dic["step_num"] * self.training_dic["large_batch_size"])
            ...
            for epoch in pbar:
                ...
                for data in self.train_loader_list[label_idx]:
                    ...
                    if not isinstance(data, list):
                        if self.training_dic.get("cross_section", False):
                            if self.training_dic.get("reduce_low_cap", False):
                                extra_feat = data[-1][0].squeeze()[:-1,:,:].permute(1,2,0)
                                reduction_cap = data[-1][0].squeeze()[-1,:,:]
                                if self.training_dic.get("reduce_method", "addition") == "addition":
                                    target_batch = (data[1].squeeze() + reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype).transpose(1,0)
                                elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                                    target_batch = (data[1].squeeze() * reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype).transpose(1,0)
                            else:
                                extra_feat = data[-1][0].squeeze()[:-1,:,:].permute(1,2,0)
                            target_batch = data[1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype).transpose(1,0)
                            if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                # print(data[0].shape, data[-1].shape)
                                data_batch = torch.cat([data[0].squeeze(), extra_feat], dim=-1).to(self.device, non_blocking=True, dtype=data[0].dtype).permute(1,0,2)
                            else:
                                data_batch = data[0].squeeze().to(self.device, non_blocking=True).permute(1,0,2)
                            if self.training_dic.get("down_weight_limited", False):
                                limited = data[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype).transpose(1,0)
                            else:
                                limited = 1
                        else:
                            if self.training_dic.get("reduce_low_cap", False):
                                extra_feat = data[-1][0][:,:-1,:].transpose(2,1)
                                reduction_cap = data[-1][0][:,-1,:]
                                if self.training_dic.get("reduce_method", "addition") == "addition":
                                    target_batch = (data[1] + reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype)
                                elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                                    target_batch = (data[1] * reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype)
                            else:
                                extra_feat = data[-1].transpose(2,1)
                                target_batch = data[1].to(self.device, non_blocking=True, dtype=data[1].dtype)
                            if self.training_dic.get("down_weight_limited", False):
                                limited = data[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype)
                            else:
                                limited = 1
                        # import ipdb; ipdb.set_trace()
                            if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                        ...
                        for chunk in data:
                            if self.training_dic.get("cross_section", False):
                                if self.training_dic.get("reduce_low_cap", False):
                                    extra_feat = chunk[-1][0].squeeze()[:-1,:,:].permute(1,2,0)
                                    reduction_cap = chunk[-1][0].squeeze()[-1,:,:]
                                    if self.training_dic.get("reduce_method", "addition") == "addition":
                                        target_batch = (chunk[1].squeeze() + reduction_cap).to(self.device, non_blocking=True, dtype=chunk[1].dtype).transpose(1,0)
                                    elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                                        target_batch = (chunk[1].squeeze() * reduction_cap).to(self.device, non_blocking=True, dtype=chunk[1].dtype).transpose(1,0)
                                else:
                                    extra_feat = chunk[-1][0].squeeze()[:-1,:,:].permute(1,2,0)
                                target_batch = chunk[1].squeeze().to(self.device, non_blocking=True, dtype=chunk[1].dtype).transpose(1,0)
                                if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                    # print(data[0].shape, data[-1].shape)
                                    data_batch = torch.cat([chunk[0].squeeze(), extra_feat], dim=-1).to(self.device, non_blocking=True, dtype=chunk[0].dtype).permute(1,0,2)
                                else:
                                    data_batch = chunk[0].squeeze().to(self.device, non_blocking=True).permute(1,0,2)
                                if self.training_dic.get("down_weight_limited", False):
                                    limited = chunk[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=chunk[1].dtype).transpose(1,0)
                                else:
                                    limited = 1
                            else:
                                if self.training_dic.get("reduce_low_cap", False):
                                    extra_feat = chunk[-1][0][:,:-1,:].transpose(2,1)
                                    reduction_cap = chunk[-1][0][:,-1,:]
                                    if self.training_dic.get("reduce_method", "addition") == "addition":
                                        target_batch = (chunk[1] + reduction_cap).to(self.device, non_blocking=True, dtype=chunk[1].dtype)
                                    elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                                        target_batch = (chunk[1] * reduction_cap).to(self.device, non_blocking=True, dtype=chunk[1].dtype)
                                else:
                                    extra_feat = chunk[-1].transpose(2,1)
                                    target_batch = chunk[1].to(self.device, non_blocking=True, dtype=chunk[1].dtype)
                                if self.training_dic.get("down_weight_limited", False):
                                    limited = chunk[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=chunk[1].dtype)
                                else:
                                    limited = 1
                            # import ipdb; ipdb.set_trace()
                                if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                    # print(data[0].shape, data[-1].shape)
                                    data_batch = torch.cat([chunk[0], extra_feat], dim=-1).to(self.device, non_blocking=True, dtype=chunk[0].dtype)
                                else:
                                    data_batch = chunk[0].to(self.device, non_blocking=True)
                            if self.training_dic.get("cross_section", False):
                                if "only_target" in self.training_dic and self.training_dic["only_target"]:
                                    target_batch = target_batch[:,-1]
                                weight = torch.ones_like(target_batch, dtype=target_batch.dtype, device=target_batch.device)
                                ...
                                if "only_target" in self.training_dic and self.training_dic["only_target"]:
                                    target_batch = target_batch[:,-1]
                                    weight = 1 + chunk[2].squeeze()[:,-1].to(self.device, non_blocking=True)
                                else:
                                    weight = 1 + chunk[2].squeeze().to(self.device, non_blocking=True)
                            with torch.no_grad():
                                pred = self.model.forward(data_batch)
                            pred = pred.detach().requires_grad_(True)
                            if "only_target" in self.training_dic and self.training_dic["only_target"]:
                    ...
                    train_loss.backward()
                    # import ipdb; ipdb.set_trace()
                    intermediate_grad = pred.grad.detach().squeeze()
                    step_cnt += 1
                    for i in range(pred.shape[0] // self.training_dic["batch_size"]):
                        ...
                        with torch.no_grad():
                            if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
                                ...
                                for data in self.eval_loader_list[label_idx]:
                                    ...
                                    if self.training_dic.get("reduce_low_cap", False):
                                        extra_feat = data[-1][0].squeeze()[:-1,:,:].permute(1,2,0)
                                        reduction_cap = data[-1][0][:,-1,:,:].squeeze()
                                        if self.training_dic.get("reduce_method", "addition") == "addition":
                                            target_batch = (data[1] + reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype)
                                        elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                                            target_batch = (data[1] * reduction_cap).to(self.device, non_blocking=True, dtype=data[1].dtype)
                                    else:
                                        extra_feat = data[-1][0].squeeze().permute(1,2,0)
                                        target_batch = data[1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype)
                                    if self.training_dic.get("down_weight_limited", False):
                                        limited = data[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype)[-1,:]
                                    else:
                                        limited = 1
                                    ...
                                    if self.training_dic.get("validation_by_return", False):
                                        ret_ratio = data[-1][1].squeeze().to(self.device, non_blocking=True, dtype=data[1].dtype)[-1,:]
                                    # import ipdb; ipdb.set_trace()
                                ...
                                for data in self.eval_loader_list[label_idx]:
                                    if self.training_dic.get("reduce_low_cap", False):
                                        ...
                                        extra_feat = data[-1][0].transpose(2,1)
                                        target_batch = data[1].to(self.device, non_blocking=True, dtype=data[1].dtype)
                                    if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                        # print(data[0].shape, data[-1].shape)
                                        data_batch = torch.cat([data[0], extra_feat], dim=-1).to(self.device, non_blocking=True, dtype=data[0].dtype)
    ...
    def evaluation(self, cfg_list):
        ...
        with torch.no_grad():
            for label_idx in range(self.num_labels):
                for data in self.eval_loader_list[label_idx]:
                    if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
                        extra_batch = data[-1][0].squeeze().permute(1,2,0)
                    else:
                        extra_batch = data[-1][0].permute(1,0)
                    if self.training_dic.get("reduce_low_cap", False):
                        if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
                            extra_feat = extra_batch[:,:,:-1]
                            reduction_cap = extra_batch[:,:,-1]
                        else:
                            extra_feat = extra_batch[:,:-1]
                            reduction_cap = extra_batch[:,-1]
                        if self.training_dic.get("reduce_method", "addition") == "addition":
                            target_batch = (data[1] + reduction_cap).to(non_blocking=True, dtype=data[1].dtype)
                        elif self.training_dic.get("reduce_method", "addition") == "multiplication":
                            target_batch = (data[1] * reduction_cap).to(non_blocking=True, dtype=data[1].dtype)
                    else:
                        extra_feat = extra_batch
    ...
    def create_raw_features(self, cfg, model_name, reader):
        ...
        if self.training_dic.get("down_weight_limited", False):
            DnLimited = reader("DnLimPrice")[buffer-1:-1,:]
            DnLimited = torch.from_numpy(all_close[buffer-1:-1,:,tidx] <= DnLimited + 1e-3).unsqueeze(0)
            UpLimited = reader("UpLimPrice")[buffer-1:-1,:]
            UpLimited = torch.from_numpy(all_close[buffer-1:-1,:,tidx] >= UpLimited - 1e-3).unsqueeze(0)
            self.limited = ~ (DnLimited | UpLimited)

        barra_indices = deepcopy(factorExp[:,buffer-1:-1,:])
        ...
        if self.training_dic.get("ignore_lim", False):
            DnLimited = reader("DnLimPrice")[buffer-1:-1,:]
            self.DnLimited = torch.from_numpy(all_close[buffer-1:-1,:,tidx] <= DnLimited + 1e-3).unsqueeze(0)
            UpLimited = reader("UpLimPrice")[buffer-1:-1,:]
            self.UpLimited = torch.from_numpy(all_close[buffer-1:-1,:,tidx] >= UpLimited - 1e-3).unsqueeze(0)
