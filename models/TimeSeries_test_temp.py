...
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        self.train_dataset = SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], train_indices, self.train_output, self.seq_len, date_weight, use_endpoints, train_endpoints, feature_matrix)
        self.eval_dataset = SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], eval_indices, self.train_output, self.seq_len, date_weight, use_endpoints_eval, eval_endpoints, feature_matrix)
        ...
        if ("discrimination" in self.training_dic and self.training_dic["discrimination"]) or ("finetune" in self.training_dic and self.training_dic["finetune"]):
            ...
            finetune_train_dataset = SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], train_indices, self.train_output, self.seq_len, np.zeros(days), True, train_endpoints, feature_matrix)
            finetune_eval_dataset = SharedMemSeqDatasetRandom(model_name, self.train_input.shape, self.train_input.dtype, indices[0], eval_indices, self.train_output, self.seq_len, np.zeros(days), True, eval_endpoints, feature_matrix)
        ...
        for epoch in pbar:
            ...
            for data in self.train_loader:
                if self.training_dic.get("reduce_low_cap", False):
                    extra_feat = data[-1][:,:-1,:].transpose(2,1)
