...
class _LinearNoInit(nn.Linear):
    """Linear layer that allocates parameters without filling them."""

    def reset_parameters(self) -> None:
        pass


class _LayerNormNoInit(nn.LayerNorm):
    """LayerNorm that allocates parameters without filling them."""

    def reset_parameters(self) -> None:
        pass


def _linear(input_dim, output_dim, device, infer):
    layer_cls = _LinearNoInit if infer else nn.Linear
    return layer_cls(input_dim, output_dim, device=device)


def _layer_norm(input_dim, device, infer):
    layer_cls = _LayerNormNoInit if infer else nn.LayerNorm
    return layer_cls(input_dim, device=device)


def _materialize_inference_tensors(module, device):
    """Materialize a meta-built module with one allocation per tensor dtype."""
    tensor_records = {}
    ...
    for submodule in module.modules():
        for name, parameter in submodule._parameters.items():
            ...
            record["owners"].append((submodule, name))
    ...
    for record in tensor_records.values():
        ...
        if tensor.layout != torch.strided:
            raise RuntimeError(
                f"infer=True only supports strided parameters and buffers, got {tensor.layout}"
            )
        records_by_dtype.setdefault(tensor.dtype, []).append(record)

    with torch.no_grad():
        for dtype, records in records_by_dtype.items():
            alignment = max(1, 256 // torch.empty((), dtype=dtype).element_size())
            offset = 0
            for record in records:
                offset = ((offset + alignment - 1) // alignment) * alignment
                record["offset"] = offset
                offset += record["tensor"].numel()

            flat_storage = torch.empty(offset, dtype=dtype, device=device)
            for record in records:
                source = record["tensor"]
                materialized = flat_storage.narrow(
                    0, record["offset"], source.numel()
                ).view(source.shape)
                if source.device.type != "meta":
                    materialized.copy_(source)
                if record["is_parameter"]:
                    materialized = nn.Parameter(
                        materialized, requires_grad=record["requires_grad"]
                    )
                    container_name = "_parameters"
                else:
                    container_name = "_buffers"
                for owner, name in record["owners"]:
                    getattr(owner, container_name)[name] = materialized

    for submodule in module.modules():
        if str(getattr(submodule, "device", "")) == "meta":
            submodule.device = torch.device(device)
...
class RandomFourierFeatures(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(torch.empty(input_dim, num_grids, device=device))
        
        # Initialize bias with uniform distribution [0, 2π]
        self.bias = nn.Parameter(torch.empty(num_grids, device=device))

        # Map to input_dim
        self.combination = _linear(2 * num_grids, input_dim, device, infer)
        ...
        if not infer:
            nn.init.normal_(self.weight, std=math.sqrt(var_w))
...
class FastKAFLayer(nn.Module):
    def __init__(
        ...
        self.layernorm = _layer_norm(input_dim, device, infer) if use_layernorm and input_dim > 1 else None

        if res_net:
            self.shortcut = _linear(input_dim, output_dim, device, infer)
        ...
        self.final_linear = _linear(input_dim, output_dim, device, infer)
...
class ChebyKANLayer(nn.Module):
    def __init__(self, input_dim, output_dim, degree, spline_weight_init_scale, dropout_rate=0.2, res_net=False, device="cpu", infer=False):
        ...
        if res_net:
            self.shortcut = _linear(input_dim, output_dim, device, infer)
            self.layernorm = _layer_norm(input_dim, device, infer) if input_dim > 1 else None
        ...
        self.register_buffer("arange", torch.arange(0, degree + 1, 1, device=device))
...
class SplineLinear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, init_scale: float = 0.1, device = 'cpu', infer=False, **kw) -> None:
...
class FastKANLayer(nn.Module):
    def __init__(
        ...
        if use_layernorm:
            ...
            self.layernorm = _layer_norm(input_dim, device, infer)
        ...
        if use_base_update:
            self.base_activation = base_activation
            self.base_linear = _linear(input_dim, output_dim, device, infer)
...
class Stem_Component(nn.Module):
    def __init__(
        ...
        self.MLP_layers = nn.ModuleList([
            _linear(stem_hidden_layer[i], stem_hidden_layer[i+1], device, infer)
            for i in range(len(stem_hidden_layer) - 1)
        ])
...
class Trident(nn.Module):
    def __init__(
        ...
        materialize_inference = (
            infer and target_device.type not in {"cpu", "meta"}
        )
        if materialize_inference:
            # Building on meta avoids one accelerator allocation per parameter.
        ...
        if time_choice_feature is not None or feature_insert.shape[0] != 0:
        ...
        self.tanh_denom = tanh_denom
        if materialize_inference:
            _materialize_inference_tensors(self, target_device)
