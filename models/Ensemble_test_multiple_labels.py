...
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        if self.tidx_incr_learning and tidx != begin_tidx:
            ...
            x, y = self.indices[1].shape
            # nb.set_num_threads(8)
            # clean_numba(self.train_input, self.indices[0], self.indices[1], n_chunks=8)
        ...
        train_extra_info = {"features": feature_matrix}
        ...
        if "regression_only" not in self.training_dic or not self.training_dic["regression_only"]:
            ...
            for i in range(self.num_buckets):
                for j in range(self.num_labels):
                    ...
                    if self.tidx_incr_learning and tidx != begin_tidx:
                        self.cfg_list[j][i].training.model_params["lr"] /= 50
                        self.cfg_list[j][i].training.model_params["retrain_lr"] /= 10
                    ...
                    for epoch in pbar:
                        ...
                        for data in self.train_loader_list[label_idx]:
                            ...
                            if self.batch_date == 1:
                                ...
                                train_loss = 0
                                for chunk in data:
                                    extra_batch = chunk[-1][0].squeeze().transpose(1,0)
                                    if self.training_dic.get("reduce_low_cap", False):
                                        extra_feat = extra_batch[:, :-1]
                                        reduction_cap = extra_batch[:, -1]
                                    else:
                                    ...
                                    if "ret" in self.training_dic.get("loss_fn"):
                                        ret_ratio = chunk[-1][1].squeeze().to(self.device, non_blocking=True, dtype=chunk[1].dtype)
                                    if self.training_dic.get("down_weight_limited", False):
                                ...
                                if self.training_dic.get("validation_by_return", False) or self.training_dic.get("cross_section", False):
                                    with torch.no_grad():
                                        ...
                                        for data in self.eval_loader_list[label_idx]:
                                            ...
                                            limited = 1
                                            if self.training_dic.get("down_weight_limited", False):
                                                limited = data[-1][-1].squeeze().to(self.device, non_blocking=True, dtype=data[0].dtype) * 2 - 1
                                            if "extra_feature" in self.training_dic and self.training_dic["extra_feature"]:
                                    ...
                                    with torch.no_grad():
                                        for data in self.eval_loader_list[label_idx]:
                                            if self.training_dic.get("reduce_low_cap", False):
                                                extra_feat = data[-1][0].squeeze()[:, :-1]
                                                reduction_cap = data[-1][:, -1]
                                            else:
                                                if isinstance(data[-1], list):
                                                    extra_feat = data[-1][0].squeeze()
                                                else:
                                                    extra_feat = data[-1]
                                            limited = 1
                                            if self.training_dic.get("down_weight_limited", False):
