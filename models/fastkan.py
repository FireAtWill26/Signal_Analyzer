...
@MODEL_REGISTRY.register('FastKAN')
                ...
                for i in range(len(self.layers)):
                    if self.time_choice_pos is not None and i == self.time_choice_pos:
                        if self.time_choice_method == "linear_modulation":
                            lm_res = self.time_choice_block(time_choice_feature)
                            x = x * lm_res[:,:self.lm_dim] + lm_res[:,self.lm_dim:]
                        elif self.time_choice_method == "barra_estimator":
                            x = x
                        else:
                            x = torch.cat([x, self.time_choice_block(time_choice_feature)], dim=1)
                    ...
                    x = self.layers[i](x)
                if self.time_choice_method == "barra_estimator":
                    a = torch.linalg.vecdot(self.time_choice_block(time_choice_feature), barra_section, dim=1).unsqueeze(1)
