...
@TRAINING_REGISTRY.register('Classification_test')
class training_test(BaseModel):
    ...
    def fit(self, cfg, indices, train_output, sample_stocks_date, sample_stocks_seccode, model_name, lock=None, *args, **kwargs):
        ...
        self.vals_path = Path(self.training_dic["model_path"]) / model_name / 'vals.pt'
        ...
        baskets, self.edges = pd.qcut(train_y, self.backet_num, labels=False, duplicates='drop', retbins=True)
        df = pd.DataFrame({'OriginalValue': train_y, 'BasketIndex': baskets})
        self.vals = torch.from_numpy(df.groupby('BasketIndex')['OriginalValue'].mean().to_numpy()).to(torch.float32).to(self.device)
        self.edges = torch.from_numpy(self.edges)[1:-1].to(torch.float32).to(self.device)
        torch.save(self.vals, self.vals_path)
        ...
        for epoch in pbar:
            ...
            for data_batch, target_batch, weight in self.train_loader:
                ...
                target_batch = torch.bucketize(target_batch, self.edges, right = False)
                ...
                if step_cnt == self.training_dic["eval_steps"]:
                    ...
                    if self.training_dic["check_mask"] and self.training_dic["eval_mask"]:
                        if not result["eval_correlation_mask"] or correlation_mask + 5 * correlation_loss> best_correlation_mask_loss:
                            best_correlation_mask_loss = correlation_mask + 5 * correlation_loss
    ...
    def evaluation(self, cfg):
        ...
        total_pred = None
        np_target = None
        for data_batch, target_batch, weight in self.eval_loader:
            if self.training_dic["denormalizer"]:
                batch_mean = torch.unsqueeze(torch.mean(data_batch, dim=1), 1)
                batch_std = torch.unsqueeze(torch.std(data_batch, dim=1), 1)
                data_batch = torch.cat([data_batch, 2*torch.tanh(batch_mean), 2*torch.tanh(batch_std)], dim=1).to(self.device, non_blocking=True)
    ...
    def load(self, model_path, model_name, **kwargs):
        ...
        self.vals_path = Path(model_path) / model_name / "vals.pt"
        return 1
    ...
    def predict(self, x, cfg, date=None, test_stage=True, lock=None, *args, **kwargs):
        ...
        model.eval()
        x = np.nan_to_num(x)
        self.vals = torch.load(self.vals_path).to(self.device)
        ...
        with torch.no_grad():
            ...
            if self.infer_mode == "max":
                pred = torch.argmax(pred, dim=1).to(torch.float32)
            elif self.infer_mode == "mean":
                pred = nn.Softmax(dim=1)(pred) * self.vals
                pred = torch.sum(pred, dim=1)
